...
# Set via configure
my $PREFIX="/g/g91/chowdhur/codes/darshan/build";

use warnings;
use lib "/g/g91/chowdhur/codes/darshan/build/lib";
use TeX::Encode;
use Encode;
...
my %hash_unique_file_read_time = ();
my %hash_unique_file_write_time = ();
my $shared_file_time = 0;
my $total_job_bytes = 0;
my $total_job_read_bytes = 0;
my $total_job_write_bytes = 0;
...
my $slowest_uniq_read_time = 0;
if(keys %hash_unique_file_read_time > 0)
{
    $slowest_uniq_read_time < $_ and $slowest_uniq_read_time = $_ for values %hash_unique_file_read_time;
}
my $slowest_uniq_write_time = 0;
if(keys %hash_unique_file_write_time > 0)
{
    $slowest_uniq_write_time < $_ and $slowest_uniq_write_time = $_ for values %hash_unique_file_write_time;
}

my $total_access_count = $read_count + $write_count;
my $est_read_meta_time = ($cumul_meta_indep + $cumul_meta_shared) * $read_count / $total_access_count;
my $est_write_meta_time = ($cumul_meta_indep + $cumul_meta_shared) * $write_count / $total_access_count;

my $avg_est_read_time = ($cumul_read_indep + $cumul_read_shared + $est_read_meta_time) / $nprocs;
my $avg_est_write_time = ($cumul_write_indep + $cumul_write_shared + $est_write_meta_time) / $nprocs;

my $total_bytes_read = $cumul_read_bytes_indep + $cumul_read_bytes_shared;
my $total_bytes_written = $cumul_write_bytes_indep + $cumul_write_bytes_shared;

print("Number of processes: $nprocs\n");
print("Total bytes read by app: $total_bytes_read\n");
print("Total bytes written by app: $total_bytes_written\n");
print("Total absolute read I/O time: $avg_est_read_time\n");
print("Total absolute write I/O time: $avg_est_write_time\n");
...
sub process_file_record
{
    ...
    if ($rank != -1)
    {
            ...
            $file_record{MPIIO_COLL_OPENS} > 0))
        {
            ...
            if(defined($hash_unique_file_read_time{$rank}))
            {
                $hash_unique_file_read_time{$rank} +=
                    $file_record{MPIIO_F_META_TIME} +
                    $file_record{MPIIO_F_READ_TIME};
            }
            else
            ...
            if(defined($hash_unique_file_write_time{$rank}))
            {
                $hash_unique_file_write_time{$rank} +=
                    $file_record{MPIIO_F_WRITE_TIME};
            }
            else
            {
        ...
        $file_record{MPIIO_COLL_OPENS} > 0) && (!($mpi_did_read)))
    {
        ...
        $total_job_write_bytes += $file_record{'POSIX_BYTES_WRITTEN'};
    }
    else
    {
        # normal case
        if(defined $file_record{'POSIX_OPENS'})
        {
            ...
            $total_job_read_bytes += $file_record{'POSIX_BYTES_READ'};
            $total_job_write_bytes += $file_record{'POSIX_BYTES_WRITTEN'};
        }
        if(defined $file_record{'STDIO_OPENS'})
        {
